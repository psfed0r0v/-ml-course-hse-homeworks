...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    feature_vector_sorted = feature_vector[idx]
    target_vector_sorted = target_vector[idx]

    where = np.diff(feature_vector_sorted) > 0
    thresholds = (feature_vector_sorted[1:][where] + feature_vector_sorted[:-1][where]) / 2

    ones = np.sum(target_vector_sorted)
    feature_length = len(target_vector_sorted)
    sums = np.cumsum(target_vector_sorted)[:-1]
    splits = np.arange(1, feature_length)

    p_left_1 = sums / splits
    p_right_1 = (ones - sums) / (feature_length - splits)
    h_left = 1 - p_left_1 ** 2 - (1 - p_left_1) ** 2
    h_right = 1 - p_right_1 ** 2 - (1 - p_right_1) ** 2

    ginis = (-1 / feature_length * (splits * h_left + (feature_length - splits) * h_right))[where]
